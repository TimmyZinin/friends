...
### Юридический анализ (завершён)

**Вердикт: Цифровое ДНК = персональные данные по GDPR и CCPA.**

Non-reversible вектор + псевдонимный идентификатор (Ed25519 pubkey) = персональные данные. Это **не** анонимные данные. Но client-side архитектура сильно упрощает compliance.

| Закон | Вопрос | Ответ | Обоснование |
|-------|--------|-------|-------------|
| **GDPR Art. 4(1)** | ДНК = personal data? | **Да** | Вектор привязан к pubkey = идентифицируемое лицо. Решение CJEU Breyer (C-582/14): даже IP = personal data |
| **GDPR Art. 9** | Special category? | **Скорее нет** | Но если чаты содержат мед. данные или политику → вектор может косвенно раскрывать. CJEU Meta Platforms (C-252/21): косвенное раскрытие = Art. 9 |
| **GDPR Recital 26** | Pseudonymized? | **Да** | Классическая псевдонимизация (Art. 4(5)). GDPR применяется, но это хорошая техническая мера (Art. 25, 32) |
| **GDPR Art. 35** | Нужен DPIA? | **Да** | Систематическое профилирование → DPIA обязательна |
| **CCPA/CPRA** | Covered? | **Да** | "Inferences drawn from personal information" (§1798.140(z)). Требует opt-out и Privacy Policy |

**Прецеденты:**
- **CNIL vs Criteo (2023):** штраф 40M EUR — профилирование через хешированные идентификаторы = обработка PD
- **AEPD (Испания):** хешированные MAC-адреса = персональные данные
- **ICO (UK, 2022):** Bloom filter с идентификатором = псевдонимные данные

### Стратегия compliance

1. **Правовая основа:** Consent (Art. 6(1)(a)) — explicit opt-in при запуске скилла
2. **DPIA (Art. 35):** Провести до публичного запуска
3. **Privacy by Design (Art. 25):** Client-side обработка = уже реализовано
4. **Прозрачность (Art. 13-14):** Privacy Policy с описанием что такое ДНК-вектор
5. **Право на удаление (Art. 17):** DELETE endpoint = реализовано в API
6. **Минимизация (Art. 5(1)(c)):** Ограничить размерность необходимым минимумом
7. **Для USA:** включить вектор в Privacy Policy как "inferences", добавить opt-out

**Ключевой вывод для Дениса:** ДНК — это персональные данные, но наша архитектура (client-side processing + pseudonymization) делает compliance реалистичным и недорогим. Это НЕ блокер, это просто checklist.
